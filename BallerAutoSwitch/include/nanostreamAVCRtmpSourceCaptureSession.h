...
/*
 * If didUpdateDisplayLayer is not implemented on app level no video frames will be rendered.
 * Sample implementation for managing the display layer on app level:
 *
- (void) didUpdateDisplayLayer: (CALayer*) layer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if([self.playerView.layer sublayers] == nil) {
            ...
            [streamInstance didUpdateDisplayLayerFinished];
        }
        else {
 ...
 */
- (id) initWithLogLevel:(nanostreamLogLevel) logLevel;
...
- (void)didUpdateDisplayLayerFinished;
