...
@synthesize outputWidth, outputHeight, name;
...
- (CIImage *)currentCIImage
{
    if (!pFrame->data[0]) return nil;
    [self convertFrameToRGB];
    return [self ciImageFromAVPicture:picture width:outputWidth height:outputHeight];
}
...
- (id)initWithVideo:(NSString *)moviePath usesTcp:(BOOL)usesTcp usesAudio:(BOOL)usesAudio withName:(NSString *)_name
{
...
//    av_dict_set(&opts, "buffer_size", "655360", 0);
    ...
    for (int i=0; i<pFormatCtx->nb_streams; i++) {
        ...
        if (usesAudio && pFormatCtx->streams[i]->codec->codec_type==AVMEDIA_TYPE_AUDIO) {
            audioStream=i;
...
- (void)dealloc
{
    ...
    // based on https://stackoverflow.com/questions/14914462/ffmpeg-memory-leak
...
- (BOOL)stepFrame
{
    ...
    while (!frameFinished && av_read_frame(pFormatCtx, &packet) >=0 ) {
        ...
        if(packet.stream_index==videoStream) {
            // free some data to prevent memory leaks
            avcodec_free_frame(&pFrame);
            pFrame = avcodec_alloc_frame();
            // Decode video frame
    ...
    [self.delegate getViewName:name withImage:image];
    
    //free packet after it's decoded
    av_free_packet(&packet);
    packet.data = NULL;
...
- (CIImage *)ciImageFromAVPicture:(AVPicture)pict width:(int)width height:(int)height
{
    CGBitmapInfo bitmapInfo = kCGBitmapByteOrderDefault;
    CFDataRef data = CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, pict.data[0], pict.linesize[0]*height,kCFAllocatorNull);
    CGDataProviderRef provider = CGDataProviderCreateWithCFData(data);
